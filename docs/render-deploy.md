...
| `CM_API_KEY` | Long-lived Bearer token for the Autodesk ingest PC (`GET /api/projects`, JSON `POST /api/documents` / `POST /api/drawings`). Do not POST file bytes to those routes — mint native B2, then ack. Do not commit. |
