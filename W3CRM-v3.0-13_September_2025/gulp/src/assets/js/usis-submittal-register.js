(function () {
	...
	function severityDot(sev) {
		if (!sev) return "—";
		var color = { critical: "#dc3545", major: "#fd7e14", minor: "#ffc107", info: "#0d6efd" }[String(sev).toLowerCase()] || "#6c757d";
		return '<span title="' + esc(sev) + '" style="display:inline-block;width:.65rem;height:.65rem;border-radius:50%;background:' + color + '"></span>';
	}
		...
		fetchJson("/api/submittals?" + qs.toString()).then(function (body) {
			...
			(body.items || []).forEach(function (row) {
				...
				trEl.innerHTML =
					...
					esc(row.status) + (row.isOverdue ? ' <span class="badge bg-danger">Overdue</span>' : "") +
					(row.rubberStampSuspect ? ' <span class="badge bg-warning text-dark">Rubber</span>' : "") +
			...
			if (!(body.items || []).length) {
				tb.innerHTML = '<tr><td colspan="12" class="text-muted text-center py-4">No submittals.</td></tr>';
			}
		}).catch(function (e) {
