/**
 * W-4 supporting document photo upload for hire wizard.
 */
(function (global) {
	"use strict";
	...
	function fileUrl(item) {
		var path = item.file_url || "/api/v1/hr/me/w4/documents/" + item.id + "/file";
		var base = apiBaseFn();
	...
	function upload(slot, file) {
		...
		fd.append("file", file);
		fd.append("slot", slot);
		return fetch(apiBaseFn() + "/api/v1/hr/me/w4/documents", {
			method: "POST",
			credentials: "include",
			body: fd,
		}).then(function (r) {
	...
	function remove(fileId) {
		return fetch(apiBaseFn() + "/api/v1/hr/me/w4/documents/" + encodeURIComponent(fileId), {
			method: "DELETE",
			credentials: "include",
	...
	function renderSlot(container, slot) {
		...
		var html =
			...
			'<p class="text-muted small mb-2">Take a photo or upload an image or PDF (front required; back optional). Up to ' +
			MAX_PER_SLOT +
			" images.</p>";
		if (items.length) {
			html += '<div class="d-flex flex-wrap gap-2 mb-2 usis-w4-doc-thumbs">';
			items.forEach(function (it, idx) {
				var label = idx === 0 ? "Front" : idx === 1 ? "Back" : "Photo " + (idx + 1);
				html +=
					'<div class="usis-w4-doc-thumb position-relative border rounded overflow-hidden">' +
					thumbHtml(it, label) +
				...
				if (!locked) {
					html +=
						'<button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1 py-0 px-1 usis-w4-doc-remove" data-file-id="' +
						esc(it.id) +
		...
		if (canAdd) {
			html +=
				...
				'<label class="btn btn-outline-primary btn-sm mb-0 usis-w4-doc-add">' +
				"Add photo or PDF" +
				'<input type="file" class="d-none" accept="' +
				FILE_ACCEPT +
				'" capture="environment" data-slot="' +
				esc(slot) +
				'">' +
				"</label>" +
				'<span class="text-muted small usis-w4-doc-status"></span>' +
				"</div>";
		} else if (locked) {
			html += '<p class="text-muted small mb-0">Document photos are locked after W-4 signature.</p>';
		}
		container.innerHTML = html;

		container.querySelectorAll(".usis-w4-doc-remove").forEach(function (btn) {
		...
		var statusEl = container.querySelector(".usis-w4-doc-status");
		if (input) {
			input.addEventListener("change", function () {
				...
				var addBtn = container.querySelector(".usis-w4-doc-add");
	...
	function getAll() {
		var out = [];
		["supporting"].forEach(function (slot) {
	...
	function wire(root, opts) {
		...
		root.querySelectorAll("[data-w4-doc-slot]").forEach(function (el) {
			var slot = el.getAttribute("data-w4-doc-slot");
