"""Replace the Penco locker family from the repo seed CSV.

Keeps unique SKU labor (6001V matches PENCO-6001V). Does not truncate other
manufacturers.

Preview:
  python scripts/replace_penco_lockers_catalog.py

Apply on Render Postgres:
  python scripts/replace_penco_lockers_catalog.py --execute --i-know-this-is-production
"""
...
CSV_PATH = _BACKEND / "data" / "catalog" / "penco_lockers.csv"
MANUFACTURER = "Penco"
...
def main() -> int:
    ...
    require_safe_execute(
        ...
        script_name="replace_penco_lockers_catalog.py",
    )
    if not args.execute:
    ...
    with app.app_context(), include_all_orgs():
        ...
        print(f"Existing Penco rows: {len(existing)}")
        if not args.execute:
        ...
        print(
            f"Replaced Penco across {len(plans)} org(s): "
