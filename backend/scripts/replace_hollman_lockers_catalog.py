"""Replace the Hollman locker family from the repo seed CSV.

Keeps unique SKU labor (A1-LAM-12x15x60 matches HOLLMAN-A1-LAM-12x15x60).
Does not truncate other manufacturers.

Preview:
  python scripts/replace_hollman_lockers_catalog.py

Apply on Render Postgres:
  python scripts/replace_hollman_lockers_catalog.py --execute --i-know-this-is-production
"""
...
CSV_PATH = _BACKEND / "data" / "catalog" / "hollman_lockers.csv"
MANUFACTURER = "Hollman"
...
def main() -> int:
    ...
    require_safe_execute(
        ...
        script_name="replace_hollman_lockers_catalog.py",
    )
    if not args.execute:
    ...
    with app.app_context(), include_all_orgs():
        ...
        print(f"Existing Hollman rows: {len(existing)}")
        if not args.execute:
        ...
        print(
            f"Replaced Hollman across {len(plans)} org(s): "
