...
| `0003_material_pricing` | `material_pricing` | `scripts\load_material_pricing.py` | `…\BOBRICK MATERIAL PRICING.CSV` (truncate) + `uPDATED PRICING.CSV` (upsert) + in-repo `data/catalog/*.csv` (upsert, e.g. Claridge visual display); use `--all-defaults` or `--repo-seeds` |
